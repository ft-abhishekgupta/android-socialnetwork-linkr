...
public class AdapterUserList extends RecyclerView.Adapter<AdapterUserList.MyViewHolder> implements Filterable {
    ...
    public void onBindViewHolder(MyViewHolder holder, int position) {
        ...
        final String currentUserEmail=FirebaseAuth.getInstance().getCurrentUser().getEmail();
            ...
            db_friendList.child(currentUserKey).child(userKey).addValueEventListener(new ValueEventListener() {
                ...
                public void onDataChange(DataSnapshot dataSnapshot) {
                    ...
                    if(dataSnapshot.hasChild("user_status"))
                    {
                        ...
                        if(s.equals("requestSent")) user_status.setText("Request Sent");
                        else if(s.equals("requestRecieved")) user_status.setText("Request Recieved");
                        else user_status.setText("Friends");
                    }
                    else
                    {
            ...
            Glide.with(context).load(listModel.get(position).getProfile_img()).thumbnail(0.5f).crossFade().centerCrop().diskCacheStrategy(DiskCacheStrategy.ALL).into(user_image);
                    ...
                    db_friendList.child(currentUserKey).child(userKey).addValueEventListener(new ValueEventListener() {
                        ...
                        public void onDataChange(DataSnapshot dataSnapshot) {
                            ...
                            if(toOpen)
                            {
                                ...
                                {
                                    String s=dataSnapshot.child("user_status").getValue().toString().trim();
                                    if(s.equals("requestSent")) openDialog("RequestSent");
                                    else if(s.equals("requestRecieved")) openDialog("RequestRecieved");
                                    else openDialog("Friends");
                                }
                                else
                                {
                    ...
                    if(friends.equals("SendRequest"))
                    {
                        ...
                        alert.setPositiveButton("Yes", new DialogInterface.OnClickListener() {
                            ...
                            public void onClick(DialogInterface dialog, int which) {
                                ...
                                ModelFriendRequest currentUserModel=new ModelFriendRequest(userKey,item_name,item_email,"requestSent");
                                ModelFriendRequest listUserModel=new ModelFriendRequest(currentUserKey,currentUserName,currentUserEmail,"requestRecieved");
                    ...
                    else if(friends.equals("RequestRecieved"))
                    {
                        ...
                        alert.setPositiveButton("Accept", new DialogInterface.OnClickListener() {
                            ...
                            public void onClick(DialogInterface dialog, int which) {
                                ...
                                db_friendList.child(currentUserKey).child(userKey).child("user_status").setValue("requestAccepted");
                                db_friendList.child(userKey).child(currentUserKey).child("user_status").setValue("requestAccepted");
                                dialog.dismiss();
                            }
                        });
